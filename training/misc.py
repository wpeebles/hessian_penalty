...
def find_pkl(results_dir, exp_number, snapshot_ix):
    if snapshot_ix == 'latest':
        ...
        snapshot_ix = '%06d' % int(snapshot_ix)
    checkpoint_paths = glob.glob('%s/%05d-*/network-snapshot-%s.pkl' % (results_dir, exp_number, snapshot_ix))
    assert len(checkpoint_paths) > 0, 'Found no pkl checkpoints in the experiment directory'
    resume_pkl = sorted(checkpoint_paths)[-1]
...
def create_image_grid(images, grid_size=None, pad_val=0, px=0, py=0):
    ...
    grid = pad_val * np.ones(list(images.shape[1:-2]) + [grid_h * img_h + py * (grid_h + 1), grid_w * img_w + px * (grid_w + 1)], dtype=images.dtype)
