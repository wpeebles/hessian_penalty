...
def create_edges_and_shoes(tfrecord_dir, image_dir, shuffle):
    print('Loading images from "%s"' % image_dir)
    image_filenames = sorted(glob.glob(os.path.join(image_dir, 'train', '*')))
    if len(image_filenames) == 0:
        error('No input images found')
    print('Found %s images' % len(image_filenames))

    img = np.asarray(PIL.Image.open(image_filenames[0]))
    channels = img.shape[2] if img.ndim == 3 else 1

    with TFRecordExporter(tfrecord_dir, 2 * len(image_filenames)) as tfr:
        order = tfr.choose_shuffled_order() if shuffle else np.arange(2 * len(image_filenames))
        for idx in range(order.size):
            ac_idx = order[idx] % len(image_filenames)
            img = PIL.Image.open(image_filenames[ac_idx])

            if order[idx] % len(image_filenames) != order[idx]:
...
def execute_cmdline(argv):
    ...
    p = add_command('create_edges_and_shoes', 'Create the Edges+Shoes dataset from the Pix2Pix dataset.',
                    'create_edges_and_shoes datasets/mydataset myimagedir')
    p.add_argument('tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument('image_dir',        help='Directory containing the train/ folder')
