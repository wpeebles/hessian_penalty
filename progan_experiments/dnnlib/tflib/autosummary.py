...
# Modified from: https://github.com/tensorflow/tensorboard/issues/39
def convert_tensor_to_gif_summary(images_arr, tag):
    summary = tf.Summary()
    ...
    clip = mpy.ImageSequenceClip(list(images_arr), fps=24)
    with tempfile.NamedTemporaryFile() as f:
        filename = f.name + '.gif'
    try:  # Handle modern versions of MoviePy:
        clip.write_gif(filename, verbose=False, logger=None)
    except TypeError:  # Handle old versions of MoviePy:
        clip.write_gif(filename, verbose=False, progress_bar=None)
...
def build_image_summary(filename, tag):
    with open(filename, 'rb') as f:
        encoded_image_string = f.read()
