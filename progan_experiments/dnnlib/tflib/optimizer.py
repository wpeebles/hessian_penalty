    ...
    def register_gradients(self, loss: TfExpression, trainable_vars: Union[List, dict]) -> None:
        ...
        with tf.name_scope(self.id + "_grad"), tf.device(dev):
            ...
            if False:
                ...
                prepro = lambda s: s[s.find('/') + 1: -2]
                G_names = ['G/', 'G_shadow/'] + ['G_shadow_%d/' % i for i in range(7)]
                grad_norm_ops = []
                for gradient, variable in grads:
                    if any([G_name in variable.name for G_name in G_names]):
                        vname = prepro(variable.name)
                        print(variable.name)
                        grad_norm_ops.append(autosummary.autosummary("Gradients/" + vname, l2_norm(gradient)))
                        grad_norm_ops.append(autosummary.autosummary("Variables/" + vname, l2_norm(variable)))
                self.grad_norm_ops = grad_norm_ops
            else:
                self.grad_norm_ops = []
